...
if(!function_exists('bbse_popup_plugin_activation')){
	function bbse_popup_plugin_activation() {
		...
		bbse_pupup_pre_action(); //pre action for network site (update 2.3.7)
...
// [Set] pre action for network site (update 2.3.7)
if(!function_exists("bbse_pupup_pre_action")) {
	function bbse_pupup_pre_action() {
		global $wpdb;

		if(substr($wpdb->prefix,0,3)!="wp_"){
			$oldPopupExits = $wpdb->get_var("show tables like 'wp_bbse_popup'");
			if($oldPopupExits){
				$wpdb->query("rename table wp_bbse_popup to ".BBSE_POPUP_DB_TABLE);
			}

			$oldAgentExits = $wpdb->get_var("show tables like 'wp_bbse_popup_agent'");
			if($oldAgentExits){
				$wpdb->query("rename table wp_bbse_popup_agent to ".BBSE_POPUP_AGENT_TABLE);
			}
		}
	}
}
...
if(!function_exists('bbse_popup_get_print')){
	function bbse_popup_get_print($pMode,$pAlias) {
		...
		foreach($result as $i=>$data) {
			if(!current_user_can('administrator') || $bbse_pid<'0' || $preview!=true){
				if (($data->popup_user=='LoggedIn' && !is_user_logged_in()) || ($data->popup_user=='NotLoggedIn' && is_user_logged_in()) || ($_COOKIE['bbsePopupIdx'.$data->idx] == 'Today' || $_COOKIE['bbsePopupIdx'.$data->idx] == 'Minute') || ($mobileCheck && $data->popup_browser=='PC') || (!$mobileCheck && $data->popup_browser=='Mobile')) continue;
			}
			...
			if($pSize['0']>$pCloseButtonLeft) $checkWidth=$pSize['0']+$pPosition['2']+50; // 팝업 왼쪽여백+버튼 왼쪽여백 포함+버튼크기
			else $checkWidth=$pPosition['2']+$pCloseButtonLeft+50; // 팝업사이즈+팝업 왼쪽여백+버튼크기
			...
			$rtnScript .="popup_view('".$pId."','".strtolower($pPosition['0'])."','','','".$checkWidth."');\n";
		}
...
if(!function_exists('bbse_popup_admin_scripts')){
	function bbse_popup_admin_scripts(){
		global $current_user, $plugin_shortname;
		$screen = get_current_screen();
		wp_get_current_user();
		$meta = get_user_meta($current_user->ID);
		$optionID=$plugin_shortname."_".$current_user->ID."_rich_editing";

		if($screen ->id=='bbs-e-popup-pro_page_bbse_Popup_pro_add' || $screen ->id=='bbs-e-popup_page_bbse_Popup_add'){
			if(!get_option($optionID)){
				update_option($optionID,$meta['rich_editing']['0']);
			}
			elseif(get_option($optionID)=='false'){
				update_user_meta($current_user->ID, 'rich_editing', 'true');
			}
		}
		else{
			if(get_option($optionID) && get_option($optionID)!=$meta['rich_editing']['0']){
				update_user_meta($current_user->ID, 'rich_editing', get_option($optionID));
			}
		}
...
if(!function_exists('uninstall_bbse_popup_plugin')){
	function uninstall_bbse_popup_plugin(){
		...
		$dropTable2="drop table if exists `".BBSE_POPUP_AGENT_TABLE."`";
		$wpdb->query($dropTable2);
	}
}
...
// [Update] rich editing
if(!function_exists('bbse_popup_update_rich_editing')){
	function bbse_popup_update_rich_editing($userID) {
		global $plugin_shortname;

		$meta = get_user_meta($userID);
		update_option($plugin_shortname."_".$userID."_rich_editing",$meta['rich_editing']['0']);
	}
}
add_action('profile_update','bbse_popup_update_rich_editing');
